...
const getMiniProfilesByPharse = async (pharse = '') => {
    try {
        const collection = await databaseService.getCollection(DB_NAME)
        const profiles = await collection.aggregate([
            ...
            {
                $addFields: {
                    profileImageId: { $toObjectId: '$relatedInfo.profileImageId' }
                }
            },
            {
                $lookup: {
        ...
        const miniProfiles = profiles.map(profile => ({
            id: profile._id,
            name: profile.entityInfo.name.display,
            branchIds: profile.relatedInfo.branchIds,
            profileImageUrl: profile.profileImage[0].entityInfo.imageUrl
        }))
        return miniProfiles
    } catch (err) {
        loggerService.error('cannot find profiles', err)
